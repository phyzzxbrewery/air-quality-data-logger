...
class DataLogger:
	...
	def _has_started_logging(self, word):
		if self.has_started_logging:
			...
			# If 1st word of dataset start logging
	...
	def readline(self):
		...
		while not bytes: 
			bytes = self._uart.readline()
		logger.debug(f'Read: {bytes}')
	...
	def readlines(self):
		...
		while n < self.no_lines:
			word = self.readline()
			if self._has_started_logging(word):
				n+=1
				yield word
			else:
				# drop word, if no start word has been catched
				continue
...
class Plantower(DataLogger):
	...
	device is completely undocumented. But the 
...
class BluetoothLogger(DataLogger):
	...
	async def read_forever(self):
		...
		async with self.ble as client:
			...
			while True:
				# Unfortunately, start_notify() returns as soon
				# as dbus has been triggered to receive messages.
				try:
	...
	async def disconnect(self):
		...
		logger.info('Bluetooth data logger is now disconnected!')
