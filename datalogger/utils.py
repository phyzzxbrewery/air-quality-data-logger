...
def get_serial_if_fname(pat):
	with open('/var/log/kern.log') as f:
		...
		while True:
			...
			p=pat.search(l)
			if p:
				logger.info(f'Found interface {p.group(1)}')
				result = '/dev/'+p.group(1)	
			if not l:
				break
		if not result:
...
class PlausiChecker:
	def process(self,dataset):
		logger.info(f'Going to check {dataset} for plausibility...')
		for k,v in dataset.items():
			"Discard dataset if one value is not in an allowed range"
			logger.debug(f"Checking {k}...")
			if k == 'temperature':
				if (not v and v != 0) or v < -30 or v > 80:
					logger.warning(f'Discarded dataset {dataset} due to invalid value for {k}')
					return
			elif k == 'pressure':
				if (not v and v != 0)  or v < 800 or v > 1200:
					logger.warning(f'Discarded dataset {dataset} due to invalid value for {k}')
					return
			elif k == 'humidity':
				if (not v and v != 0)  or v > 100 or v <  0:
					logger.warning(f'Discarded dataset {dataset} due to invalid value for {k}')
					return
			else:
				if v < 0 or (not v and v != 0):
